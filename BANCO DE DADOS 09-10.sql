...
/*QUAL E O VALOR TOTAL DO PEDIDO  NUMERO 7*/
/*SUM*/
...
right join pedidos on pedidos.cliente_id = clientes.id;
 ...
 -- quantidades de pedidos realizados por cliente, exibir nome do cliente quantidade de pedidos realizados--
...
select * from pedidos;
   ...
   inner join clientes on pedidos.cliente_id = clientes.id where pedidos.data >'2016-12-15';
   
   
   -- 4.Quantos pedidos foram feitos pelo cliente com o nome "Alexandre Santos"?
 select nome, count(pedidos.id) from clientes left join pedidos on clientes.id = pedidos.cliente_id 
 where nome = 'Alexandre Santos' group by nome;
 ...
 -- 6.Qual valor total de todos os pedidos feitos até agora
 select sum(valor) from itens_pedido;
...
-- 8. Liste todos os clientes e seus pedidos feitos no mês de dezembro de 2016 (inclua clientes sem pedidos*)
 ...
 inner join pedidos on pedidos.clientes_id = clientes.id 
...
select pedidos.id as cliente, nome, pedidos.id as pedidos from clientes left join pedidos on pedidos.cliente_id = cliente_id; 

-- 14. Qual o pedido mais caro, exibir o nome do cliente e o valor do pedido.
select pedidos.id as cliente, nome, pedidos.id as pedidos, MAX(valor) from clientes left join pedidos on pedidos.cliente_id = cliente_id; 
